...
interface StudentMessagesProps {
  ...
  uploadMessageFile?: (params: { schoolId: string; fileUri: string; fileName: string; mimeType?: string } | { uri: string; type: string; name?: string }) => Promise<string>;
...
export const StudentMessages: React.FC<StudentMessagesProps> = ({
  ...
  const sendFile = useCallback(async (uri: string, type: 'image' | 'document', name?: string) => {
    ...
    try {
      let uploadedUrl = '';
      if (typeof uploadMessageFile === 'function') {
         // Handle both parameter styles found in SchoolDataContext
         try {
           uploadedUrl = await (uploadMessageFile as any)(currentSchoolId, uri, name || `attachment_${Date.now()}`, type);
         } catch {
           uploadedUrl = await (uploadMessageFile as any)({ uri, type, name });
         }
      }
      await handleSendMessage(type, uploadedUrl, name || `attachment_${Date.now()}`, undefined, selectedChat);
    } catch {
      HapticPatterns.error();
    ...
    <View className="flex-1 bg-gray-50 w-full">
      <View className="flex-1 relative w-full">
        <Animated.View className="absolute inset-0 z-10 w-full h-full" pointerEvents={selectedChat ? 'none' : 'auto'} style={inboxAnimStyle}>
          <PlatinumSearchHeader title="Scholar Sync" subtitle="Academic Exchange" searchValue={searchQuery} onSearchChange={setSearchQuery} placeholder="Search faculty..." />
          <FlatList 
