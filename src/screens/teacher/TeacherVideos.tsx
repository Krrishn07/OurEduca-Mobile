...
export const TeacherVideos = React.memo<TeacherVideosProps>(({
  ...
  const { dbCameraNodes: rawCameraNodes, fetchCameraNodes } = useSchoolData();
      ...
      {!isStreaming ? (
        <View className="px-4 pt-6 space-y-6">
          ...
          <AppCard className="p-5 border border-gray-100 shadow-sm">
            ...
            <Text className="text-gray-400 text-[10px] font-black uppercase tracking-[1px] mb-6 font-inter-black">Choose your stream source</Text>

            <View className="space-y-3">
              {[
                { id: 'CAMERA', icon: '📹', label: 'Device Camera',     desc: 'Stream from your phone or tablet camera' },
                { id: 'CCTV',   icon: '📷', label: 'Connect Security Feed', desc: 'Link classroom security via Stream Address' },
            ...
            {streamSource === 'CCTV' && (
              <View className="mt-4 bg-sky-50 p-4 rounded-2xl border border-sky-100">
                <Text className="text-sky-700 text-[10px] font-black uppercase tracking-[1px] mb-2 font-inter-black">Security Feed Address</Text>
                ...
                <Text className="text-sky-600/60 text-[9px] font-medium font-inter-medium mt-2">Enter the classroom Security Feed address or stream link.</Text>
              </View>
            )}
          </AppCard>
          ...
          <AppCard className="p-5 border border-gray-100 shadow-sm">
            ...
            <Text className="text-gray-400 text-[10px] font-black uppercase tracking-[1px] mb-4 font-inter-black">Select a class to go live</Text>
              ...
              {teacherAssignedSections.map((cls: any, i: number) => (
                <View key={i as any} className={`flex-row items-center py-3 ${i < teacherAssignedSections.length - 1 ? 'border-b border-gray-50' : ''}`}>
                  <View className="flex-1">
                    ...
                    <Text className="text-gray-400 text-[10px] font-black uppercase tracking-[1px] mt-1 font-inter-black">{cls.subject} • Room {cls.room_no}</Text>
                  </View>
                  <TouchableOpacity 
              ...
              <View className="flex-1 h-screen bg-slate-950">
                <View className="m-0 overflow-hidden bg-slate-950 shadow-2xl relative flex-1">
                   ...
                   <View style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0 }}>
                      ...
                      {/* HUD: Top Bar */}
                      <View 
                        className="absolute left-6 right-6 z-20 flex-row justify-between items-start"
                        style={{ top: insets.top + 10 }}
                      >
                        <View>
                          <View className="bg-live-indicator px-3 py-1.5 rounded-full flex-row items-center shadow-lg border border-live-indicator/50">
                            <Animated.View style={{ opacity: pulseAnim }} className="w-1.5 h-1.5 bg-white rounded-full mr-2" />
                            <Text className="text-white text-[8px] font-inter-black uppercase tracking-[2px]">LIVE</Text>
                          </View>
                          
                          {/* Subject & Status */}
                          <Text className="text-white font-inter-black text-xl tracking-tight mt-3">
                            {streamSubject}
                          </Text>
                          <Text className="text-white/50 text-[10px] font-inter-black uppercase tracking-[1px]">
                            {streamSource === 'CAMERA' ? 'Integrated Hardware' : 'CCTV Uplink'}
                          </Text>
                        </View>

                        <View className="items-end">
                          {/* Timer Node */}
                          <View className="bg-black/60 px-4 py-2 rounded-2xl border border-white/10 backdrop-blur-md items-center">
                            <Text className="text-live-indicator font-inter-black text-lg tracking-tight">
                              {formatTime(elapsed)}
                            </Text>
                            <Text className="text-white/30 text-[7px] font-inter-black uppercase tracking-[1px]">
                              RECORDING
                            </Text>
                          </View>
                          ...
                          <View className="bg-white/10 px-2 py-1 rounded-full mt-3 flex-row items-center">
                              ...
                              <Text className="text-white text-[8px] font-inter-black uppercase tracking-[1px] ml-1">
                                {viewerCount} Viewers
                              </Text>
                          </View>
                        </View>
                      ...
                      <View className="absolute bottom-10 left-6 right-6 z-30">
                          <View className="bg-white/10 backdrop-blur-3xl p-4 rounded-[40px] border border-white/10 flex-row items-center gap-4 shadow-2xl">
                             ...
                             <TouchableOpacity 
                               ...
                               className="bg-live-indicator px-8 py-5 rounded-[28px] shadow-2xl shadow-live-indicator/40 active:bg-rose-600 active:scale-95 transition-all"
                             >
        ...
        <View className="space-y-4">
          {filteredVideos.length > 0 ? (
            ...
            <View className="py-20 items-center justify-center">
              ...
              <Text className="text-gray-400 text-[10px] font-black text-center px-10 leading-relaxed font-inter-black uppercase tracking-[1px]">
  ...
  const renderMonitorTab = () => {
    return (
      <ScrollView className="flex-1 px-4 pt-4" showsVerticalScrollIndicator={false}>
         <Text className="text-[9px] font-black text-gray-400 uppercase tracking-[3px] mb-4 px-2 font-inter-black">Institutional Classroom Views</Text>
         ...
         <View className="space-y-3">
             {memoizedCameraNodes.map((node: any, i: number) => (
                <TouchableOpacity 
                  key={node.id || i}
                  activeOpacity={0.9}
                  className="bg-white border border-gray-100 rounded-[28px] p-4 mb-3 flex-row items-center shadow-sm"
                >
                  {/* Live Visualizer Node */}
                  <View className="w-12 h-12 bg-slate-900 rounded-2xl items-center justify-center mr-4 overflow-hidden border border-slate-800">
                    ...
                    <Animated.View 
                      style={{
                        ...
                        position: 'absolute',
                        transform: [{ translateY: scanlineAnim.interpolate({ inputRange: [0, 1], outputRange: [-24, 24] }) }]
                      }} 
                    />
                  </View>

                  <View className="flex-1">
                    <Text className="text-gray-900 font-inter-black text-[15px] tracking-tight">
                      {node.name || `Classroom View ${i+1}`}
                    </Text>
                    <View className="flex-row items-center mt-1">
                      <View className="w-1.5 h-1.5 rounded-full bg-emerald-500 mr-2 shadow-sm shadow-emerald-500/40" />
                      <Text className="text-emerald-600 text-[10px] font-inter-bold uppercase tracking-[1px]">
                        Live Feed
                      </Text>
                    </View>
                  </View>
                  ...
                  <View className="items-end">
                     <View className="bg-indigo-50 px-4 py-2 rounded-xl border border-indigo-100">
         ...
         <View className="mt-6 px-6 py-4 bg-gray-50 rounded-[24px] border border-gray-100 items-center">
            <Icons.Shield size={16} color="#94a3b8" />
            <Text className="text-gray-400 text-[9px] font-inter-bold uppercase tracking-[1px] mt-2 text-center leading-4">
              Authorized Personnel Only • Recorded Session Policy Applies
            </Text>
         </View>
          ...
          <View className="px-4 py-4">
            <View className="flex-row bg-white/50 p-1 rounded-[20px] border border-white shadow-sm">
              {(() => {
                ...
                if (currentUser?.role === 'mentor' || currentUser?.role === 'teacher') tabs = ['STREAM', 'MONITOR', 'LIBRARY']; 
                return tabs;
              })().map(s => (
                <TouchableOpacity 
                  key={s}
