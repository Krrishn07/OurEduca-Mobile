...
export const TeacherClassDetail = React.memo<TeacherClassDetailProps>(({
  ...
  // Reactive IDs
  const classId = useMemo(() => selectedClass?.class_id || selectedClass?.id, [selectedClass]);
  const rosterId = useMemo(() => selectedClass?.rosterId || selectedClass?.id, [selectedClass]);
  ...
  useEffect(() => {
    if (students.length > 0 && classId) {
      // FIX: Only store students belonging to this specific class/section
      const filtered = students.filter(s => s.class_id === classId && (s.section ?? 'A') === (selectedClass?.section ?? 'A'));
      setLocalStudents(filtered);
      saveToCache(classId, 'students', filtered);
    }
  }, [students, classId, selectedClass?.section]);

  useEffect(() => {
    if (materials.length > 0 && classId) {
      const filtered = materials.filter(m => m.class_id === classId);
      setLocalMaterials(filtered);
      saveToCache(classId, 'materials', filtered);
    }
  }, [materials, classId]);

  useEffect(() => {
    if (assignments.length > 0 && classId) {
      // FIX: Only store assignments belonging to this specific class
      const filtered = assignments.filter(a => a.class_id === classId);
      setLocalAssignments(filtered);
      saveToCache(classId, 'assignments', filtered);
    }
  }, [assignments, classId]);
  ...
  const classStudents = useMemo(() => {
    return students.filter(s => s.class_id === classId && (s.section ?? 'A') === (selectedClass?.section ?? 'A'));
  }, [students, classId, selectedClass?.section]);
  ...
  const classAssignments = useMemo(() => {
    return assignments.filter(a => {
      ...
      return aClassId === classId || aClassId === selectedClass?.id || aClassId === selectedClass?.rosterId;
    });
  }, [assignments, classId, selectedClass]);

  const activeData = useMemo(() => {
    // FIX: Use prop-filtered memoized values directly instead of local state to ensure reactivity
    let data = activeTab === 'ROSTER' ? classStudents : activeTab === 'MATERIALS' ? classMaterials : classAssignments;
    
    // Fallback to local state if props are empty (e.g. offline cache)
    if (data.length === 0) {
      data = activeTab === 'ROSTER' ? localStudents : activeTab === 'MATERIALS' ? localMaterials : localAssignments;
    }
  ...
  }, [activeTab, debouncedQuery, classStudents, classMaterials, classAssignments, localStudents, localMaterials, localAssignments]);
                  ...
                  <View className={`flex-row items-center justify-between px-4 py-3.5`}>
                    ...
                    <View className="ml-3">
                      {isRoster ? (
                        ...
                        >
                          <Text className="text-white text-[10px] uppercase tracking-[1px] font-inter-black">Grade</Text>
                        </Pressable>
                      )}
                    </View>
                  </View>
