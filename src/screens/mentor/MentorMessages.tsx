...
export const MentorMessages: React.FC<MentorMessagesProps> = ({
  selectedChat,
  setSelectedChat,
  msgInput,
  setMsgInput,
    ...
    <View className="flex-1 bg-[#f5f7ff]">
      {!selectedChat ? (
        <View className="flex-1">
          {/* Platinum Inbox Header — 140px Sync */}
          <StyledLinearGradient
            colors={AppTheme.colors.gradients.brand}
            start={{ x: 0, y: 0 }}
            end={{ x: 1, y: 1 }}
            className="h-[140px] px-6 pt-5 rounded-b-[40px] shadow-xl shadow-indigo-200/50 z-20"
          >
            ...
            <View className="relative z-10 mb-5">
              ...
              <Text className={`${AppTypography.eyebrow} text-white/60 mt-1.5`}>Institutional Exchange Hub</Text>
            </View>
              ...
              {(displayContacts || []).map((contact, idx) => {
                ...
                return (
                  <AppRow
                    key={contact.id}
                    title={contact.name}
                    subtitle={lastMsg?.content || lastMsg?.message || "No message history..."}
                    avatarLetter={contact.name?.charAt(0) || '?'}
                    avatarBg="#f0f2ff"
                    avatarColor="#4f46e5"
                    meta={lastMsg ? formatAcademicTime(lastMsg.created_at) : undefined}
                        ...
                        <View className={`max-w-[85%] px-5 py-4 rounded-[28px] shadow-2xl shadow-indigo-100/40 ${
                            ...
                            <View className="flex-row items-center justify-end mt-2.5 opacity-60">
                                <Text className={`text-[9px] font-black uppercase tracking-widest font-inter-black ${isMine ? 'text-indigo-100' : 'text-gray-400'}`}>
                                    {msg.timestamp || formatAcademicTime(msg.created_at)}
                                </Text>
