...
import { dbRoleToUserRole, userRoleToDbRole } from '@utils/roleUtils';
...
export interface SchoolDataContextType {
  ...
  fetchSystemLogs: (schoolId?: string, offset?: number) => Promise<void>;
...
export const SchoolDataProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const fetchSystemLogs = useCallback(async (schoolId?: string, offset: number = 0) => {
    try {
        let query = supabase
            .from('system_logs')
            .select('*')
  ...
  const fetchMessages = useCallback(async (schoolId: string, force = false) => {
    ...
    try {
        const { data, error } = await supabase
            ...
            .order('created_at', { ascending: false })
            .limit(200); // Cost Optimization: Only latest 200 for previews
  ...
  const fetchMoreMessages = useCallback(async (schoolId: string, partnerId: string, offset: number) => {
    if (!schoolId || !currentUser?.id || !partnerId) return;
    try {
        const { data, error } = await supabase
            .from('messages')
            .select('*')
        ...
        console.error('fetchMoreMessages error:', err.message);
    }
  }, [currentUser?.id]);

  // Realtime Handshake: Global Messaging Subscription
  useEffect(() => {
    if (!currentUser?.id) return;

    console.log('[REALTIME] Initializing message channel for user:', currentUser.id);
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          setChatMessages(prev => {
            ...
            return [payload.new as ChatMessage, ...prev];
          });
        }
      )
      .on(
        ...
        (payload) => {
          console.log('[REALTIME] Message received update:', payload.new.id);
      ...
      .on(
        ...
        (payload) => {
          console.log('[REALTIME] My message was read/updated:', payload.new.id);
          setChatMessages(prev => prev.map(m => m.id === payload.new.id ? { ...m, ...payload.new } : m));
        }
      )
      .subscribe();
  ...
  const contextValue = useMemo(() => ({
      ...
      fetchInstitutes, registerInstitute, sendChatMessage, uploadMessageFile, fetchMessages, fetchMoreMessages, logSystemActivity,
