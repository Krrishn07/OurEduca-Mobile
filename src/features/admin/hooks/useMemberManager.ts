...
export const useMemberManager = (
  ...
  const handleSaveClass = useCallback(async (className: string, sections: string, editingClassId: string | null, subject?: string, roomNo?: string, classTime?: string) => {
    if (!className || !mockAuthUser?.school_id) return;
    try {
      setLoading(true);
      const sectionArray = sections.split(',').map(s => s.trim()).filter(s => s !== '');
      
      const classPayload: any = { name: className, sections: sectionArray };
      if (subject) classPayload.subject = subject;
      if (roomNo) classPayload.room_no = roomNo;
      if (classTime) classPayload.class_time = classTime;
      ...
      if (editingClassId) {
        ...
        classPayload.school_id = mockAuthUser.school_id;
        const { error } = await supabase
          .from('classes')
          .insert(classPayload);
        if (error) throw error;
