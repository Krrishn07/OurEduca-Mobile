...
import { AppTheme, AppCard, AppRow, StatusPill, SectionHeader } from '../../../design-system';
...
export const TeacherProfile: React.FC<TeacherProfileProps> = ({
  ...
  return (
    <ScrollView className="flex-1 bg-[#f5f7ff]" showsVerticalScrollIndicator={false}>
      {/* Hero Banner - Dynamically padded for the notch */}
      <View className="relative z-10">
        <StyledLinearGradient
          colors={AppTheme.colors.gradients.brand}
          start={{x: 0, y: 0}} end={{x: 1, y: 1}}
          style={{ paddingTop: insets.top, height: 140 + insets.top }}
          className="rounded-b-[40px] shadow-xl shadow-indigo-200/50"
        />
      </View>

      {/* Avatar & Header Info - Flowing together naturally */}
      <View className="px-6 relative z-20 -mt-10 mb-6">
        <View className="flex-row justify-between items-end">
          {/* Avatar pulled up by negative margin on parent */}
          <View className="p-1.5 rounded-[24px] bg-white shadow-xl shadow-indigo-200 border border-gray-50 self-start">
            <View className="w-20 h-20 rounded-[20px] bg-indigo-50 items-center justify-center border border-indigo-100 overflow-hidden shadow-inner relative">
              ...
              {/* Online Status */}
              <View className="absolute bottom-1 right-1 w-4 h-4 rounded-full bg-emerald-500 border-2 border-white shadow-sm" />
            </View>
          </View>
          ...
          <TouchableOpacity
            onPress={onEdit}
            className="bg-white border border-gray-100 shadow-md shadow-indigo-100/20 p-3 rounded-xl active:scale-95 mb-2"
          >
            <Icons.Edit size={18} color="#4f46e5" />
          </TouchableOpacity>
        </View>

        {/* Text Block directly underneath */}
        <View className="mt-4">
          <Text className="text-xl text-gray-900 tracking-tighter leading-none mb-2 font-inter-black">
            {currentUser?.name}
          </Text>
        ...
        <View className="px-4 mb-24">
            ...
            <View className="mt-16 items-center opacity-30">
                ...
                <Text className="text-[9px] text-gray-400 uppercase tracking-[4px] font-inter-black">Verified Faculty Node</Text>
                <Text className="text-[8px] text-gray-400 mt-1 uppercase tracking-widest font-inter-black italic">TLS 1.3 SECURE CONNECTION</Text>
            </View>
        </View>
    </ScrollView>
  );
